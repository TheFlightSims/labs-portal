...
from subconfig import DatabaseConnectionString
...
# Private and Public SSL Certificates
# Default as the paths that the installation script places them
pub_key:            str = environ.get("JHUB_PUB_KEY").strip()
pri_key:            str = environ.get("JHUB_PRI_CERT").strip()

# JupyterHub Proxy Secrets
# Default as the paths that the installation script places them
proxy_auth_token:   str = environ.get("JHUB_PROXY_AUTH_TOKEN").strip()
cookie_secret:      str = environ.get("JHUB_COOKIE_SECRET").strip()

# Default Shell for users
# Default to /bin/bash if not set
def_shell:          str = environ.get("JHUB_DEFAULT_SHELL").strip()

# Allowed regular users and admin users/groups
def_allowed_users:  str = environ.get("JHUB_ALLOWED_USERS").strip()
def_admin_users:    str = environ.get("JHUB_ADMIN_USERS").strip()
def_admin_group:    str = environ.get("JHUB_ADMIN_GROUP").strip()

# Set for each user limits
cpu_limit:          int = int(environ.get("JHUB_CPU_LIMIT"))
mem_limit:          str = environ.get("JHUB_MEM_LIMIT").strip()

# Database connection settings
db_username:        str = environ.get("JHUB_DB_USERNAME").strip()
db_password:        str = environ.get("JHUB_DB_PASSWORD") # Should not strip the password
db_host:            str = environ.get("JHUB_DB_HOST").strip()
db_name:            str = environ.get("JHUB_DB_DATABASE").strip()
db_port:            str = environ.get("JHUB_DB_PORT").strip()
db_dialect:         str = environ.get("JHUB_DB_DIALECT").strip()
db_driver:          str = environ.get("JHUB_DB_DRIVER").strip()
...
##################################################################################################
# Object Initialize for setting
dbConnectionObj = DatabaseConnectionString(
    dialect = db_dialect,
    driver = db_driver,
...
if db_type == 'postgresql':
    dbConnectionString = dbConnectionObj.get_postgres_connection_url()
elif db_type == 'mysql':
    dbConnectionString = dbConnectionObj.get_mysql_connection_url()
elif db_type == 'oracle':
    dbConnectionString = dbConnectionObj.get_oracle_connection_url()
elif db_type == 'mssql':
    dbConnectionString = dbConnectionObj.get_mssql_connection_url()
...
## Instead of starting the Application, dump configuration to stdout (Note: DO NOT ENABLE IT)
#  Default: False
# c.Application.show_config = False

## Instead of starting the Application, dump configuration to stdout (as JSON) (Note: DO NOT ENABLE IT)
#  Default: False
...
#  Default: 'C:\\Users\\ShiroiNeko\\miniconda3\\envs\\jhub_base\\share\\jupyterhub'
# c.JupyterHub.data_files_path = 'C:\\Users\\ShiroiNeko\\miniconda3\\envs\\jhub_base\\share\\jupyterhub'
...
c.JupyterHub.db_url = dbConnectionString

## log all database transactions. This has A LOT of output
#  Default: False
c.JupyterHub.debug_db = db_debug
